...
class Game {
    ...
    private void initGamePanel(int Rsize, int Csize) {
        ...
        for(int ridx=0; ridx<Rsize; ++ridx){
            for(int cidx=0; cidx<Csize; ++cidx){
                ...
                Tile tile = new Tile(this,currentCoord);
                tiles.add(tile);
            }
        }
    }
    ...
    void clickOn(coordinate coord) {
        getTileBycoordinates(coord).incTileAttackPoints();
        getTileBycoordinates(new coordinate(coord.x-1,coord.y)).incTileAttackPoints();
        getTileBycoordinates(new coordinate(coord.x+1,coord.y)).incTileAttackPoints();
        getTileBycoordinates(new coordinate(coord.x,coord.y+1)).incTileAttackPoints();
        getTileBycoordinates(new coordinate(coord.x,coord.y-1)).incTileAttackPoints();
    }
}
